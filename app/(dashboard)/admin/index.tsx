...
export default function AdminOverviewScreen() {
  ...
  const { data: metrics, isLoading: isMetricsLoading } = useQuery({ queryKey: ['admin_metrics'], queryFn: getDashboardMetrics });
  const { data: leaderboard, isLoading: isLeaderboardLoading } = useQuery({ queryKey: ['admin_leaderboard'], queryFn: getLeaderboard });
  const { data: shapData, isLoading: isShapLoading } = useQuery({ queryKey: ['admin_shap'], queryFn: getShapData });
  const { data: chartsData, isLoading: isChartsLoading } = useQuery({ queryKey: ['admin_charts'], queryFn: getChartsData });
      ...
      <View style={styles.contentWrapper}>
        ...
        <FadeInView delay={100} translateY={-10}>
          <View style={styles.headerRow}>
            <View>
              <Text style={[styles.pageTitle, { color: theme.text }]}>Global State Overview</Text>
              <Text style={[styles.pageSub, { color: theme.textSecondary }]}>Maharashtra Pollution Control Board • All 14 Regions</Text>
            </View>
            <View style={styles.badgeRow}>
              <View style={[styles.badge, { backgroundColor: theme.primaryBg, borderColor: theme.primaryBorder }]}>
                <Text style={[styles.badgeTextBlue, { color: theme.primary }]}>SYSTEM HEALTH: OPTIMAL</Text>
              </View>
            </View>
          </View>
        </FadeInView>
        ...
        <View style={isMobile ? styles.gridCol : styles.gridRow}>
          ...
          <FadeInView delay={400} translateY={15} style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.border, flex: isMobile ? 0 : 2 }]}>
            <View style={styles.cardHeader}>
              <Text style={[styles.cardTitle, { color: theme.text }]}>Global AI Detection Trends</Text>
            ...
            <Text style={[styles.chartSubText, { color: theme.textSecondary }]}>Aggregate tamper probability across state-wide CEMS data</Text>
            <ErrorBoundary>
        ...
        <View style={isMobile ? styles.gridCol : styles.gridRow}>
          <FadeInView delay={600} translateY={20} style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.border, flex: isMobile ? 0 : 1 }]}>
            <Text style={[styles.cardTitle, { color: theme.text }]}>Primary Tamper Methods Detected</Text>
            <ErrorBoundary>
              ...
              {['Navi Mumbai', 'Tarapur', 'Pune', 'Nagpur'].map((region, i) => (
                <View key={i} style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, flex: 1, marginLeft: 20 }}>
                    <View style={{ flex: 1, height: 8, backgroundColor: theme.background, borderRadius: 4 }}>
                      <View style={{ width: `${85 - i * 15}%`, height: 8, backgroundColor: i > 1 ? theme.warning : theme.success, borderRadius: 4 }} />
                    </View>
                    <Text style={{ color: theme.textSecondary, width: 40, textAlign: 'right' }}>{85 - i * 15}%</Text>
                  </View>
                </View>
              ))}
            </View>
...
const styles = StyleSheet.create({
  ...
  badgeTextBlue: { fontSize: 12, fontWeight: '900' },
